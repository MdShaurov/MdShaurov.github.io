...
# Purpose: To create the Chrome Dinosaur game using pygame.
#######################################################
...
# Locating all the files required
...
# Background clouds characteristics
class Cloud:
    def __init__(self):
...
# Setting up moving across screen for obstacles that appear
class Obstacle:
...
# Setting postion and image for cactus
...
# Setting animation and position for bird
class Bird(Obstacle):
    def __init__(self, image):
        self.type = 0
        super().__init__(image, self.type)
...
def menu(death_count):
    ...
    while run:
        ...
        # Screen after losing
            ...
            if points > highscore:
                highscore = points
                open(os.path.join("Assets/Other", "scores.txt"), "w").write(str(highscore))
            SCREEN.blit(GAMEOVER, (SCREEN_WIDTH // 2 - 195, SCREEN_HEIGHT // 2 - 140))
            SCREEN.blit(RESET, (SCREEN_WIDTH // 2 - 40, SCREEN_HEIGHT // 2 - 60))
            pygame.display.update()
...
# Getting past highest scores
highscoreGet = open(os.path.join("Assets/Other", "scores.txt")).readlines(0)
highscore = int(highscoreGet[0])
